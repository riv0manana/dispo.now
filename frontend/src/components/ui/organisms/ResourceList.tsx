...
export function ResourceList({ resources, onEdit, onDelete, onCreate }: ResourceListProps) {
    ...
    <Card>
      <table className="w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-zinc-800">
          {resources.map((res) => (
            <tr key={res.id} className="group hover:bg-zinc-900/30 transition-colors">
              ...
              <td className="px-6 py-4 text-zinc-400 text-xs">
                {res.bookingConfig?.daily?.start ? `${res.bookingConfig.daily.start}-${res.bookingConfig.daily.end}` : '24h'}
